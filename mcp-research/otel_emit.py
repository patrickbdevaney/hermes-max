...
Self-contained (mirrors mcp-observability) so mcp-research stays an INDEPENDENT
process. If Phoenix is down, spans are dropped silently and `record` returns
{"ok": False, "exported": False} — it NEVER raises and never blocks the agent.

Research spans: research_planned, queries_developed, sources_explored,
claims_verified, report_synthesized, echo_chamber_blocked, low_authority_filtered,
deep_research_done.
"""
...
def _init() -> None:
    ...
    try:
        ...
        try:
            ...
            _exporter_ok = False
        _tracer = provider.get_tracer("hermes-max-research")
