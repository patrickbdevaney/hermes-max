...
# Install a thin WRAPPER (not a copy): the real run-evolution.sh needs its repo
# siblings (evolve.py, traces.py, .venv), so the cron script delegates to it by
# absolute path. Re-running just refreshes the wrapper's target.
cat > "${HERMES_SCRIPTS}/run-evolution.sh" <<EOF
#!/usr/bin/env bash
# Auto-generated by dspy-evolution/register-cron.sh — delegates to the repo script.
exec bash "${SCRIPT_DIR}/run-evolution.sh" "\$@"
EOF
chmod +x "${HERMES_SCRIPTS}/run-evolution.sh"
echo "installed wrapper: ${HERMES_SCRIPTS}/run-evolution.sh -> ${SCRIPT_DIR}/run-evolution.sh"
